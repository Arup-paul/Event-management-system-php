...
<div id="main-content" class="main-content">
		<div class="page-title">
				...
				<h1><i class="fa fa-plus"></i> New Service  </h1>
			
		</div>
			<?php
				if (isset($_GET['edit'])) {
					$id=$_GET['edit'];
					$edit_query="SELECT * FROM service WHERE id='$id'";
					if($run_edit=mysql_query($edit_query)){
						while($edit_row=mysql_fetch_array($run_edit)){
							...
							<label><strong>Link</strong></label>
								<input class="form-control" type="text" value="detail.php?type=news&id='.$id.'&headline='.$sname.'" readonly>
							</div>
							<div class="form-group col-lg-12 col-sm-12 col-md-12 col-xs-12" style="max-height:200px;overflow:hidden;">
								...
								<div class="form-group  col-lg-12 col-sm-12 col-md-12 col-xs-12">
									<textarea id="summernote" class="form-control" rows="5" name="pdetails" >'.$sdetails.'</textarea>
								</div>
				...
				<div class="col-lg-12 ">
					<textarea id="summernote" class="form-control" rows="5" name="pdetails" ></textarea>
				</div>
				...
				<hr><br>
				<div class="form-group col-lg-12 col-sm-12 col-md-12 col-xs-12">
				      <button type="submit" name="service_insert" class="btn btn-md btn-primary" value="Insert Information"><i class="fa fa-plus"></i>  Submit Post</button>
				</div>
			</form>
					';
				}
						...
						while ($we_care_main2=mysql_fetch_assoc($we_care_main1)) { 
							...
							echo '
							 <tr>
							    ...
							    <td>
								    <a href="service.php?edit='.$empid.'&type='.$pstatus.'">EDIT</a>
								</td>
								<td>
								    <a href="service.php?delete='.$empid.'&type='.$pstatus.'">DELETE</a>
								</td>
						      </tr>
							';
...
<?php
	...
	if(isset($_POST['service_insert'])){
		...
		if(empty($pname) || empty($who_image)){	
			...
			$pro_query="INSERT INTO service (name,address,pstatus,image,details,date) 
	...
	if(isset($_POST['service_update'])){
		...
		if(empty($who_image_check)){	
			...
			$pro_query="UPDATE service SET name='$pname',address='$paddress',details='$detail' WHERE id='$pid'";
			if($run_pro_query=mysql_query($pro_query)){
			...
			$pro_query="UPDATE service SET name='$pname',address='$paddress',image='$who_image',details='$detail' WHERE id='$pid'";
			if($run_pro_query=mysql_query($pro_query)){
	...
	if(isset($_GET['delete'])){
		$pid=$_GET['delete'];
		$edit_query="SELECT * FROM service WHERE id='$pid'";
		if($run_edit=mysql_query($edit_query)){
			while($edit_row=mysql_fetch_array($run_edit)){
				$image=$edit_row['image'];
				unlink('../../images/upload/'.$image);
			}
			$del_query="DELETE FROM service WHERE id='$pid'";
