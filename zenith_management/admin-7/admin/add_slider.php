...
<div id="main-content" class="main-content">
		<div class="page-title">
				...
				<h1><i class="fa fa-picture-o"></i>Add New Slider</h1>
			
		</div>


		
			<form action="add_slider.php" method="POST" role="form" enctype="multipart/form-data" class="col-lg-6">
				...
				<div class="form-group col-lg-12 col-sm-12 col-md-12 col-xs-12 ">
				      <input type="text" class="form-control caption_text" name="pstatus" placeholder="event name ">
				</div>


				<div class="form-group col-lg-12">
					     <label for="">Image insert high Resoulation</label>
						...
						while ($we_care_main2=mysql_fetch_assoc($we_care_main1)) { 
							...
							echo '
							 <tr>
						        ...
						        <td>'.$pstatus.'</td>
							 <td>
						            <a href="add_slider.php?delete='.$empid.'&type='.$pstatus.'">DELETE</a>
							</td>
						      </tr>
							';
...
<?php
	...
	if(isset($_POST['picup'])){
		...
		if (empty($gallery_name)) {
			...
			$insert ="INSERT INTO add_slider(gallery_name,caption,image,type) VALUES ('$gallery_name','$caption','$image','image')";
	...
	// 		$insert ="INSERT INTO add_slider(gallery_name,caption,image,vurl,type,promo) VALUES ('$gallery_name','$caption','$image','$vurl','video','$promo')";
	// 		if($insert_run=mysql_query($insert)){
	// 			if($promo==1){
	...
	if(isset($_GET['delete'])){
		$pid=$_GET['delete'];
		$edit_query="SELECT * FROM add_slider WHERE id='$pid'";
		if($run_edit=mysql_query($edit_query)){
			while($edit_row=mysql_fetch_array($run_edit)){
				$image=$edit_row['image'];
				unlink('../../images/upload/'.$image);
			}
			$del_query="DELETE FROM add_slider WHERE id='$pid'";
